...
def build_analysis_prompt(request: AnalyzeRequest, include_preparation: bool = True) -> str:
    context = request.candidateContext
    stack = ", ".join(context.currentStack)
    preparation_context = (
        f"- Preparation plan length: {request.preparationPlanDays} days"
        if include_preparation
        else "- Preparation plan length: not requested in this call"
    )
    seven_day_plan_schema = (
        """
  "sevenDayPlan": [
    {
      "day": number,
...
"""
    ...
    seven_day_plan_rule = (
        f"- sevenDayPlan must contain exactly {request.preparationPlanDays} day(s).\n"
        f"- day must start at 1 and end at {request.preparationPlanDays}."
        if include_preparation
        else "- sevenDayPlan must be an empty array because preparation is generated by a separate endpoint."
    )
    coaching_instruction = (
        "- Generate resume improvements, interview questions, and cross-questions."
        if include_preparation
        else (
            "- Do not generate resume improvements, interview questions, or cross-questions in this call. "
            "Return empty arrays for resumeImprovements, interviewQuestions, and crossQuestions."
        )
    )
    resume_improvements_schema = (
        """
  "resumeImprovements": [
    {
      "currentIssue": string,
...
"""
    ...
    coaching_rules = (
        ...
        if include_preparation
        else "- resumeImprovements, interviewQuestions, and crossQuestions must be empty arrays."
    )
